...
describe('The Happy Path', () => {
  it('Does Everything', () => {
    const testdata = {
      ...
      task2_description: "This is the second tasks cypress will add, and it is higher priority",
      task2_priority: "6",
    ...
    cy.get('.TableTask').then((tasks) => {
      ...
      expect(taskNames.indexOf(testdata.task2_name)).to.be.lessThan(taskNames.indexOf(testdata.task1_name));
    });
    ...
    cy.contains('.fc-event-title.fc-sticky', testdata.task2_name)
      .should('be.visible');
    ...
    cy.get('.TableTask').then((tasks) => {
      ...
      expect(taskNames.indexOf(testdata.task1_name)).to.be.lessThan(taskNames.indexOf(testdata.task2_name));
    });
